#' Clustering Cumulative Incidence Functions in competing risks framework
#'
#' @description Function for grouping cumulative incidence curves (CIF) based on
...
clustcif <- function(time, status = NULL, x, max_time = NULL, labels = NULL,
 ...
 # weights <- status
  ...
  res <- list(num_groups = k, table = data.frame(H0 = h0tested, Tvalue = tval, pvalue = pval),
              ...
              call = match.call())
#  class(res) <- "clustcurves" #antes era asi
  class(res) <- "clustcif"
  return(res)
